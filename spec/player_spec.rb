...
describe 'ComputerAI module' do
  ...
  describe 'XComputer module' do
    describe '#x_second_move' do
      ...
      context '"X" has the center and "O" has a side' do
        it 'gives any move but the one directly across from "O"' do
      ...
      context '"X" took a corner first and "O" takes a side' do
        it 'takes the center or a corner' do
      ...
      context '"X" took a corner first and "O" also took a corner' do
        it 'takes one of the two remaining corners' do
          x_computer.send(:board=, x_started_in_a_corner_and_o_took_a_corner)
          expect(x_computer.second_move).to eq([:top_left, :bottom_left])
        end
      end

      context '"X" took a corner first and "O" is in the center' do
        it 'moves anywhere' do
      ...
      context '"X" took a side first and "O" is in a corner or the center' do
        it 'moves anywhere besides the opposite side' do
    ...
    describe '#x_third_move' do
      context '"O" is on a side with an empty neighbor' do
        it 'takes an empty neighbor' do
          x_computer.send(:board=, o_left_a_neighbor_open)
          expect(x_computer.x_third_move).to eq([:top_left, :top_right])
        end
      end

      context '"O" is only in the corners' do
        it 'takes a corner, any corner' do
          x_computer.send(:board=, x_gotta_corner_on_o)
          expect(x_computer.x_third_move).to eq([:top_left, :top_right, :bottom_right])
        end
      end
    end
  end
...
#### X THIRD MOVES ####
  def o_left_a_neighbor_open
    { top_left:    " ", top_center:    "O", top_right:    " ",
      middle_left: "X", center:        " ", middle_right: " ",
      bottom_left: "O", bottom_center: "X", bottom_right: " " }
  end

  def x_gotta_corner_on_o
    { top_left:    " ", top_center:    "X", top_right:    " ",
      middle_left: "X", center:        "O", middle_right: " ",
      bottom_left: "O", bottom_center: " ", bottom_right: " " }
  end
